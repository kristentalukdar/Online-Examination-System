...
public class UserController {
	...
	@GetMapping("/vaidateuser/{UserType}/{RegNo}/{Password}")
	public User get(@PathVariable String UserType, @PathVariable String RegNo, @PathVariable String Password) {
		System.out.println(UserType);
		String uType=UserType.toLowerCase();
		if (uType.equals("admin")) {
			return userService.validateAdminUser(RegNo, Password);
		}else if (uType.equals("teacher")) {
			return userService.validateTeacherUser(RegNo, Password);
		}else if (uType.equals("student")) {
			return userService.validateStudentUser(RegNo, Password);
		}
		return null;
	};
